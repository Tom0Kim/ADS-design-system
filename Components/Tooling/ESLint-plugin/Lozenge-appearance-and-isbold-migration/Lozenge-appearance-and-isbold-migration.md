# Lozenge appearance and isbold migration

Source page: https://atlassian.design/components/eslint-plugin-design-system/lozenge-appearance-and-isbold-migration
...
This rule helps migrate deprecated `` usages to the new API or `` component, and
migrate `` component appearance prop to new semantic values as part of the Labelling System
Phase 1 migration.
...
1. **Migrate `isBold={false}` or missing `isBold` → `> Embedded documentation component: `Tag` (see the original MDX under `_source`).`**: Detects non-bold Lozenge variants
...
Both `` and `` now use the `appearance` prop with new semantic values. The rule will
...
- ✅ `isBold={false}` and missing `isBold` migration to `` with preserved `appearance` prop
