...
Source page: https://atlassian.design/components/eslint-plugin-design-system/lozenge-badge-tag-labelling-system-migration
...
This rule helps migrate deprecated `` props and appearance values to the new API, migrate
`` component appearance prop to new semantic values, and migrate `` and
`` components to the new `` or `` components as part of the Labelling
...
1. **Remove deprecated `isBold` prop from ``**: The `isBold` prop is deprecated in the new
...
6. **Migrate SimpleTag/RemovableTag with Avatar to AvatarTag**: Detects SimpleTag and RemovableTag
   ...
   `AvatarTag` component with render props pattern: `avatar={(props) => > Embedded documentation component: `Avatar` (see the original MDX under `_source`).}`.
...
The `` component stays as `` — it no longer migrates to ``.
...
- ✅ SimpleTag/RemovableTag migration to `` with color mapping and import updates
- ✅ SimpleTag/RemovableTag from subpaths (e.g., `@atlaskit/tag/simple-tag`) migration to main
  module
- ✅ SimpleTag/RemovableTag with Avatar migration to `` with render props pattern
